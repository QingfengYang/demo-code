...
object ReduceIDSetApp {
  ...
  def main(args: Array[String]): Unit = {
    ...
    var (notRelated_op, relatedDS_op) = reduceSimple(idMapRdd)
    reducedIDMapDS ++= notRelated_op.get
    while (relatedDS_op.isDefined && relatedDS_op.get.size > 1) {
      var (notRelated_op_tmp, relatedDS_op_tmp) = reduceSimple(relatedDS_op.get)
  ...
  def reduceSimple(idMapRdd: List[IdMapSet]): (Option[List[IdMapSet]], Option[List[IdMapSet]]) = {
    ...
    val flatten_setId_rdd: List[(String, IdMapSet)] = groupedWf.values.map(idMapSet => idMapSet.setId.toString -> idMapSet).toList
